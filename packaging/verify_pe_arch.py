...
def main() -> int:
    ...
    for path in args.root.rglob("*"):
        ...
        if architecture is None:
            continue
        checked += 1
        if architecture not in {expected, "anycpu"}:
            failures.append((path, architecture))

    if failures:
        for path, architecture in failures:
            print(f"{path}: expected {expected} or AnyCPU, found {architecture}")
        return 1

    print(f"Verified {checked} PE files as {expected} or AnyCPU")
    return 0
